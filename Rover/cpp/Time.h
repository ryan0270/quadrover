...
class Time
{
	public:
		...
		uint64 getNS(){return mTime.tv_sec*1e9+mTime.tv_nsec;}
		uint64 getUS(){return mTime.tv_sec*1e6+mTime.tv_nsec/1.0e3;}
		uint64 getMS(){return mTime.tv_sec*1e3+mTime.tv_nsec/1.0e6;}

		uint64 getElapsedTimeUS() {Time now; return (now.mTime.tv_sec-mTime.tv_sec)*1e6+(now.mTime.tv_nsec-mTime.tv_nsec)/1.0e3; }
		uint64 getElapsedTimeMS() {Time now; return (now.mTime.tv_sec-mTime.tv_sec)*1e3+(now.mTime.tv_nsec-mTime.tv_nsec)/1.0e6; }
