...
namespace Quadrotor{
using namespace std;
class TrackedPoint : public enable_shared_from_this<TrackedPoint>
{
	public:
	TrackedPoint();
	TrackedPoint(std::vector<cv::Point> points);

	void copyData(const TrackedPoint &ao);
	...
	void addNeighbor(shared_ptr<TrackedPoint> n, bool doTwoWay);
	...
	const vector<shared_ptr<TrackedPoint>> &getNeighbors() const {return mNeighbors;}
	...
	static TNT::Array2D<double> calcCorrespondence(const vector<shared_ptr<TrackedPoint>> &prevObjectList,
	...
	static double calcShapeDistance(const shared_ptr<TrackedPoint> &ao1, const shared_ptr<TrackedPoint> &ao2);
	...
	vector<shared_ptr<TrackedPoint>> mNeighbors;
...
{
	public:
	std::shared_ptr<TrackedPoint> aoPrev, aoCur;
	float score;
};
