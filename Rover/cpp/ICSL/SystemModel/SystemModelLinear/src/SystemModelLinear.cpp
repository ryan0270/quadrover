...
namespace ICSL{
	SystemModelLinear::SystemModelLinear()
	{
	}
	...
	bool SystemModelLinear::loadFromString(char *buff)
	{
		mxml_node_t *root = mxmlLoadString(NULL,buff,MXML_TEXT_CALLBACK);
		if(root == NULL)
		{
			Log::alert("SystemModelLinear::loadfromString == xml load failed");
			return false;
		}
	...
	int SystemModelLinear::serialize(Collection<tbyte> &serial)
	{
	...
	void SystemModelLinear::deserialize(Collection<tbyte> const &serial)
	{
