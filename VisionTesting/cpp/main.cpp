...
void loadPhoneLog(String filename, 
				...
				list<shared_ptr<Data<int>>> &fastThresholdList
				);
template <class T>
...
//	imgDir = "../video_Aug16_3";
	...
	list<shared_ptr<Data<int>>> fastThresholdList;
	loadPhoneLog(String(imgDir.c_str())+String("/phoneLog.txt"), imgIdList, attDataListOrig, transDataListOrig, errCovDataListOrig, motorCmdsDataListOrig, thrustDirDataListOrig, fastThresholdList);
	if(imgIdList.size() == 0)
	{
...
cout << "start time = " << curTime.getMS() << endl;
		...
		int fastThreshold = IData::interpolate(curTime, fastThresholdList);
...
//				covVel = matmult(rotCamToPhone, matmult(covVel, rotPhoneToCam));
			...
			if(imgPrev.data != NULL)
			{
				...
				stringstream ss;
				ss << imgDir << "/annotatedORB/img_" << imgCnt++ << ".bmp";
...
//				cv::circle(img, pt+center, 4, cv::Scalar(255,0,0), -1);
			...
			stringstream ss;
			ss << imgDir << "/annotatedFAST/img_" << imgCnt++<< ".bmp";
//			imwrite(ss.str(), img);
			imshow("chad",img);
		...
		while(file.good())
		{
			...
			switch(type)
			{
				...
				case LOG_ID_MOTOR_CMDS:
					...
					break;
				case 5002: // FAST threshold
					{
						int thresh;
						ss >> thresh;
						shared_ptr<Data<int>> data(new Data<int>());
						data->timestamp.setTimeMS(time);
						data->data = thresh;
						fastThresholdList.push_back(data);
					}
					break;
			}
		}
